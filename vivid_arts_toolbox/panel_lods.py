...
class VIVID_PT_setup_lods(Panel):
    ...
    def draw(self, context):
        ...
        if s:
            ...
            sp_box.label(text="ShadowProxy High Ratios")
            sp_col = sp_box.column(align=True)
            sp_col.prop(s, "sp_lod0_ratio", text="SP High LOD0")
            sp_col.prop(s, "sp_lod1_ratio", text="SP High LOD1")
            sp_col.prop(s, "sp_lod2_ratio", text="SP High LOD2")
            sp_col.prop(s, "sp_lod3_ratio", text="SP High LOD3")

            # ShadowProxy Low toggle and per-LOD ratios (vertical)
            settings_col.prop(s, "generate_low_shadow_proxies", text="Generate Low ShadowProxies")
            sp_low_box = settings_col.box()
            sp_low_box.label(text="ShadowProxy Low Ratios")
            sp_low_col = sp_low_box.column(align=True)
            sp_low_col.prop(s, "sp_low_lod0_ratio", text="SP Low LOD0")
