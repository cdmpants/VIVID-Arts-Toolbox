...
- `vivid_arts_toolbox/vivid_painter_export.py` — file-driven Painter export flow; important helper functions: `run_export`, `_find_optimized_obj`, `_proj_dirs`. Use these to reproduce behavior in unit tests or integration scripts.
...
- Many functions require a saved .blend file and scene objects; to test `vivid_painter_export.run_export` run inside Blender with an active object named `*_Optimized` and a saved .blend (see `_proj_dirs`).
...
- Registration: modules with `register()`/`unregister()` (e.g., `vivid_painter_export.py`, `operators/*.py`) should be imported in `__init__.py` and wired in the top-level `register()`/`unregister()` order.
...
- To read preferences: in an Operator use `prefs = context.preferences.addons[PACKAGE].preferences` (pattern used in `vivid_painter_export.py`).
- To copy packaged templates: use `_addon_dir()` helper (see `vivid_painter_export._addon_dir`) and `Path` operations to keep OS-safe file handling.
